...
export function extractColorValue(props) {
  ...
  const allColorsKeys = [..._keys(Colors), ..._keys(schemeColors)];
  const colorPropsKeys = Object.keys(props).filter(key => _includes(allColorsKeys, key));
...
export function extractBackgroundColorValue(props) {
  ...
  const bgProp = _findLast(keys, prop => Colors.getBackgroundKeysPattern().test(prop) && !!props[prop]);
