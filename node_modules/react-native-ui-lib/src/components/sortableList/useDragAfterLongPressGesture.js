...
const useDragAfterLongPressGesture = props => {
  const {
    onDragStart,
    onDragUpdate,
    onDragEnd,
  ...
  const longPressGesture = Gesture.LongPress().minDuration(150).onStart(() => {
    isLongPressed.value = true;
  }).onEnd(() => {
    isLongPressed.value = false;
  });
  const dragGesture = Gesture.Pan().manualActivation(true).onTouchesMove((_event, stateManager) => {
    if (!isDragging.value) {
